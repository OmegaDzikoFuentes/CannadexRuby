...
class Achievement < ApplicationRecord
  ...
  def add_progress!(amount = 1)
    ...
    # Pass user directly — no join needed now
    achievement_progresses.create!(progress_amount: amount, user: user)
  end

  def unlock!
    update!(is_unlocked: true, unlocked_at: Time.current)

    # Use the new award_xp! helper instead of update_column
    user.award_xp!(xp_reward)
    ...
    Notification.deliver_to(
      ...
      body:       description,
      notifiable: self,
      data:       { xp_earned: xp_reward }
    )
  end

  def claim!
    return unless is_unlocked? && !is_claimed?
    update!(is_claimed: true, claimed_at: Time.current)
  end
end