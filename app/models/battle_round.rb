...
class BattleRound < ApplicationRecord
  ...
  # round_results is now jsonb — access keys directly, no more JSON.parse
  def results
    round_results || {}
  end

  def challenger_score
    results['challenger_score'] || 0.0
  end

  def opponent_score
    results['opponent_score'] || 0.0
  end
  ...
  def strains_are_different
    errors.add(:opponent_strain, "must be different from challenger strain") if
      challenger_strain == opponent_strain
  end
